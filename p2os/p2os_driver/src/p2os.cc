...
P2OSNode::check_and_set_gripper_state()
{
 ...
 // grip_command[0] = GRIPPER;
  ...
  //grip_command[3] = 0;
  grip_command[0] = DIGOUT;
  grip_command[1] = ARGINT;
  if (grip_val == 1){
    grip_command[2] = 0x02; //DIGOUT //open gripper
    grip_command[3] = 0x03;
  }
  else if (grip_val == 2){
    grip_command[2] = 0x00; //DIGIN //close gripper
    grip_command[3] = 0x03;
  }  
  else if (grip_val == 3){
    grip_command[2] = 0x08; //UP
    grip_command[3] = 0x0c;
  }
  else if(grip_val == 4){
    grip_command[2] = 0x00; //DOWN
    grip_command[3] = 0x0c;
  }
  else if(grip_val == 5){
    grip_command[2] = 0x01; //stop gripper
    grip_command[3] = 0x01;
  }
  else if(grip_val == 6){
     grip_command[2] = 0x04; //stop lift
    grip_command[3] = 0x04;
  }
  else {//7
    grip_command[2] = 0x05; //stop all
    grip_command[3] = 0x05;
  }
  printf("grip command\n");
